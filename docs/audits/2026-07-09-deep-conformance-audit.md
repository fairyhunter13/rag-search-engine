...
| HR27/28/29 docgen/OKF kill-switches | Read `kb/docgen.py::run_docgen`/`kb/okf.py::run_okf` (both gate on `RSE_DOCGEN`/`RSE_OKF` env vars, default on); federation members are cleaned via `_cleanup_generated_docs`, never generated; confirmed `docgen`/`okf` absent from `_MCP_TOOLS` | Clean — matches existing test coverage (`test_docgen_hierarchy_e2e.py`, `test_docgen_all_projects.py`, `test_okf.py`) |
