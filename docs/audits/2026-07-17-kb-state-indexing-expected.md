...
Every parked project matched this shape (e.g. go-monorepo `head=51 enriched=40`, 11 silently dropped).
...
- `redacted-name-0` (105 communities): `kb_state=ready`, `enriched_pct=100.0` — **converged from
...
- `ts_fleet` (19 communities): `enriching`, `52.6%` — mid-convergence, awaiting its next daemon
  ...
  regression. Will converge when enrich next runs, as `redacted-name-0` did.
