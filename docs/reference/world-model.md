...
> **Status:** reference — repo-agnostic. This document defines *what a world model is* (independent of any specific repo). For OSE's governance/spec world model see `docs/world-model/`. For redacted-name-10's governance+execution world model see `redacted-name-10-project/docs/world-model/`.
...
| **redacted-name-10-project** | **Governance/spec + execution** — governance/spec layer (repo invariants) **plus** an executable domain world model. State = `SimState` (commitments/call-offs/returns/transfers/inspections/invoices/SOH). Actions = domain commands (CreateCommitment, PostCallOff, …). Planner = BFS `plan(state, goal)`. Validator = `--validate` mode in `simulate.py`. | `docs/world-model/simulate.py` |

**One shared `model.yaml` schema, two profiles.** `scripts/gen_world_model_skills.py` and `scripts/check_world_model.py` are parameterized per repo (governance-only profile for OSE; governance+execution profile for redacted-name-10) — one tool, two profiles, not two separate generators.
...
- `redacted-name-10-project/docs/world-model/SPEC.md` — redacted-name-10 governance+execution WM narrative
- `redacted-name-10-project/docs/world-model/simulate.py` — redacted-name-10 executable planner + validator
