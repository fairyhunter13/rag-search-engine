...
6. `reconstruct_processes(root_path)` (Phase D BPRE) — runs ONLY for federation roots (≥2 members).
   ...
   resolution ladder's Tier 1/1.5/1.75/2 confidence tiers above, which resolve individual
   ...
   cloud DeepSeek for D5 rule text + D6 narrative (built and active). D4 process traces are **handler-anchored and deduped** — keyed
