...
The governing principle is **most efficient + most effective, for *everything* in RSE**. Every component, lane, and algorithm is chosen and tuned for maximum efficiency *and* effectiveness; all principles below are corollaries (§9b per-workload engine assignment, no cross-lane bleed; HR6, HR9, HR10, HR12, HR26).

1. **The climb from source chunks to invariants is deterministic at every rung, and costs `$0`.** This is the stronger successor to the token-economy principle that held this slot until the cloud generative lane retired on 2026-07-28 — a budget only allocates while some rung has a price. The id is kept rather than reused.
2. **tree-sitter, and nothing beneath it — no dynamic or static mapping, no keyword, no regex.** The only code that classifies *what the user's code means* uses tree-sitter (structural facts). There is no LLM tier underneath to catch what tree-sitter cannot statically reach, which makes the prohibition load-bearing rather than tidy: a heuristic added here would be the *only* answer, not a first guess. No regex, no static/dynamic keyword list, no mapping table may substitute for structural analysis of user code, **package-wide over `src/rag_search/`** outside the intrinsic-mechanism files (§7a; HR15).
3. **GPU-only inference; CPU fallback fatal; maximize GPU, minimize CPU & RAM** (HR6, HR26, HR32, HR40). Idle target: < 1 % CPU, constant RAM floor (models unload after `RSE_MODEL_IDLE_UNLOAD_S`). The heavy pass — graph re-derive + structural community labelling — runs only on real source-fingerprint drift (`_code_source_fingerprint` gate in `on_change`; the `_last_enriched_sig` gate it replaced went with the enrich/wiki/BPRE cascade on 2026-07-28, and the gate survived because the re-derive it protects is the expensive half that did). File-watching is event-driven via `watchfiles`/Rust `notify` — one thread + one inotify instance for all roots, `watch_filter` ignore-aware via the same HR35 resolver as the drift gate (HR33, HR37); polling, if ever needed, is the Rust library's own `force_polling` path, never a hand-rolled loop.
4. **No local generative LLM, and since 2026-07-28 no cloud one either except chat** — `claude -p` at claude-haiku-4-5 on the dashboard chat route is the *whole* generative surface, and `routes_chat.py` is its only caller. The cloud DeepSeek KB lane and the doc-tooling lane both left with tier 3 (HR12, HR31).
5. **Determinism + idempotence** — byte-identical reruns, unconditionally: there is no LLM left to turn off. Community labelling is gated on `summary IS NULL` and never re-labels settled rows (HR3, HR20, HR21, HR24, HR25).
...
9. **Two-stage retrieval; rerank is the relevance authority.** Query = hybrid recall (bi-encoder `sqlite-vec` + FTS5 BM25, fused by RRF) → cross-encoder rerank (`gte-reranker-modernbert-base`, GPU); results ordered by `rerank_score`, **never the bare retrieval score**; **both** AXIS A (code chunks) and AXIS B (community/architecture context) are reranked; reranking runs **at query time only**, never at index/KB-build time (HR8).
10. **Public-repo hygiene, whole-tree.** Every tracked file must be safe to publish: no secrets, no real device paths, no company or device names (HR34). RSE emits no artifacts, so there is nothing narrower to scope the rule to — and the hazard the old artifact-scoped rule protected against is still live: `symbols.file` stores **absolute** paths, so anything that ever renders one must strip it to root-relative first.
11. **Engineering doctrine** — every line of code is a liability (prefer no change → deletion → smallest sufficient diff); correctness before speed; live suite uses no mocks (real embedder + GPU). The §13b→§14 map from each invariant to its live test closes the V&V loop, and `test_coverage_map_names_resolve` gates the map itself.

## 1b. Where the rules live
...
There is no separate invariant register, and adding one back is the change this section exists to
refuse: `docs/decisions/2026-08-14-the-register-was-a-sixth-copy.md` records what the last two cost.
...
## 7a. Code-semantic classification doctrine — tree-sitter, and nothing beneath it (HR15)
