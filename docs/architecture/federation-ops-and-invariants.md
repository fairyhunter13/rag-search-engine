...
| **HR30** | **Concept→Spec→Impl→Test traceability is machine-verified** (2026-06-25). `kb/world_model.py` (GPU-free parse) reads §1a principles, §13b HR table, §14 test map, and `FEATURES.md`; hard-gates on orphan HR (no §1a citation), phantom test ref (test name absent from `src/tests/live/`), dead code anchor (file path absent under `src/`), and dangling principle HR ref. Fuzzy FEATURES→HR linkage is embed+abstain (report-only, never flips verdict). Verdict + coverage + validation stamp surfaced at `overview(what='world_model')`. |
...
| HR21 e2e + HR20 metamorphic (June-2026 live validation) | HE1 live L3 build ≤8 themes; HE2 overview(hierarchy) cross-surface; HE3 member_count structural identity; HE4 no-clobber idempotency; HE5 cross-project quality invariants; HE6 two-clique positive oracle; HE7 `test_owning_root_l3_refresh_is_wired` source-guard + behavioral; HE8 FaithLens faithfulness (@slow); HE9 cross-model haiku judge (@slow); HE10 ask(scope=global) end-to-end (@slow) | `test_hierarchy_e2e.py` |
...
| HR30 world model V&V | `test_no_doctrine_orphan_hr`, `test_no_dangling_principle_hr`, `test_no_untested_hr`, `test_no_phantom_test_ref`, `test_no_dead_code_anchor`, `test_structural_validate_is_valid`, `test_feature_coverage_reported`, `test_write_validation_stamp` | `test_world_model_traceability.py` |
