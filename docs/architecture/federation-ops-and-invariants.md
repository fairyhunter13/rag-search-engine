...
| **HR14** | **BPRE process graph is a root-level artifact**: `process_graph.db` lives in `root_process_db(root_path)` (the federation root's index dir), never in any per-member `graph.db` (HR4 preserved). `reconstruct_processes(root_path)` runs only for roots with ≥2 members. The deterministic pass (D2 entry points, D3 gRPC+Pub/Sub+HTTP cross-service resolution via shared astro-proto contract, D4 process tracing BFS depth-6, D6 BPMN 2.0 XML + sequenceDiagram) is **GPU-free** and byte-identical on repeated runs (`OSE_WIKI_LLM=0` skips narrative). Cloud DeepSeek is used only for D5 rule-text extraction and D6 process narrative. Surfaces: `overview(what='process_flows')` returns reconstructed processes when db exists; `GET /api/process/bpmn` exports XSD-valid XML; the dashboard Processes view renders sequenceDiagrams via Mermaid. |
