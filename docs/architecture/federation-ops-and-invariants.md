...
| **HR30** | **MCP surface integrity** (2026-06-26, Phase 2c; guard re-pointed 2026-07-28). The MCP tool surface must be exactly `index + search + graph + overview` (`ask` retired 2026-07-29). *(Old HR30 = `kb/world_model.py` RTM was deleted WS-B; identifier re-used.)* Guard: `test_mcp_has_four_tools` in `test_server.py` (set equality, not the subset check it used — a subset cannot see an *extra* tool, so the "only" was never enforced) — `test_docgen_dashboard.py`, which used to hold the negative half of this, is deleted along with the routes it guarded. |
