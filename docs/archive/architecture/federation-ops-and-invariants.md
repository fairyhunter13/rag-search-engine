...
| **HR29** | **`.rse-index.yaml` honored by every project-source enumerator.** **Two enumerators exist, and they cannot disagree**: the bulk walk (`index/discover.py::iter_files`) and the incremental/watcher path (`daemon/sweeps.py::on_change` → `discover.py::is_ignored_path`). Both resolve through the single `_should_drop` predicate in `discover.py`, which is where it lives precisely so the watcher, the indexer and the drift signal cannot diverge; `effective_config`/`is_excluded` survive as its `.rse-index.yaml` tier. That shared predicate is the whole of HR29's surface — a narrower claim than a list of walks, and a stronger one, because it is structural rather than repeated. No enumerator may silently index a file that `effective_config` would exclude. (See §16.5 in Part 1.) A second silence closed 2026-08-17: the *loader* dropped an unknown block, a misspelled key, a non-string list member and a quoted `"false"` without a word, which is how `watcher.max_pending_files` was parsed, inherited and reported at `_overview.py` for months while enforcing nothing. `load_project_config` now raises `ProjectConfigError` with a `difflib` suggestion, `effective_config` catches it and quarantines the one project with `exclude=["*"]` rather than taking the watcher down over a typo, and `config_error(root)` carries the reason to `overview(status)` — the only surface on which a quarantined project is visible at all. Guards: HH6–HH9 in `test_config_universality.py`, and SC11, which requires every `ProjectConfig` field to have a reader outside the loader and outside the reporting surface. |
...
| §16 config inheritance | `test_hh1_full_index_baseline`, `test_hh2_on_change_filters_excluded`, `test_hh3_code_walk_honours_exclude`, `test_hh4_code_walk_universal_discovery`, `test_hh5_is_code_language_contract`; the `federation.exclude` field's inheritance is `test_fe3_discover_members_skips_excluded_repo` and `test_fe6_glob_entry_excludes_member`, which run against a root config and assert on a member *(re-pointed 2026-07-28 — all three former tests left with `test_p22_kb_e2e.py`)* | `test_config_universality.py`, `test_federation_exclude.py` |
...
| HR29 config universality — every enumerator | `test_hh1_full_index_baseline`, `test_hh2_on_change_filters_excluded`, `test_hh3_code_walk_honours_exclude`, `test_hh4_code_walk_universal_discovery`, `test_hh5_is_code_language_contract`, `test_hh6_a_well_formed_config_still_loads`, `test_hh7_a_key_the_loader_does_not_know_is_refused`, `test_hh8_a_wrongly_typed_value_is_refused`, `test_hh9_a_broken_config_quarantines_its_project_instead_of_raising`, `test_sc11_every_project_config_field_has_a_consumer` *(loader validation added 2026-08-17; re-pointed 2026-07-28 — HH3's "cross-surface" pair was the spine and BPRE walks; the surviving cross-surface claim is that every enumerator routes through HR35's `_should_drop`)* | `test_config_universality.py` |
...
| Federation exclusion in force on the live daemon (§15.3) | `test_fe8_the_effective_exclusion_is_non_empty`, `test_fe9_no_enabled_project_is_federation_excluded`, `test_fe10_armed_disabled_rows_are_covered_by_the_exclusion`, `test_fe11_the_gates_fire_when_the_exclusion_is_lost` (sufficiency); both sources: `test_fe3_discover_members_skips_excluded_repo`, `test_fe4b_config_form_carries_the_same_grammar`, `test_fe6_glob_entry_excludes_member`, `test_fe12_register_all_members_disables_an_excluded_row`; declared-vs-in-force: `test_se6_every_declared_federation_exclusion_is_in_force`; drop-in wiring: `test_systemd_dropins_target_deployed_unit` | `test_federation_exclude.py`, `test_systemd_effective_config.py`, `test_daemon.py` |
...
The separate worktree exclusion §15.3 covers joined that same file on 2026-08-17, under
`federation.exclude` rather than `index.exclude`: `index.exclude` filters files *inside* a member,
and a worktree symlink resolves to a directory outside the root that discovery registers as a member
in its own right.
...
**Sequel: versioning the file was not enough, because env is the wrong carrier (2026-08-17).** FE8
proves the value is in force *in the daemon*, and that is the whole population it can speak for. The
CLI, this repo's own live suite and every operator script are processes systemd never handed the
variable to, so each of them runs `register_all_members()` with the exclusion empty and re-enables
all 58 rows — which is how those rows came back *after* the drop-in was installed and in force. The
layout half of the value (`*/_worktrees/*`) therefore moved into the federation root's
`.rse-index.yaml` under a new `federation.exclude` block, which `_federation_exclude_entries` unions
with the environment; every process reads a file. The environment keeps only what a tracked file
cannot hold — absolute host paths, kept out by HR34 — and `federation-exclude.conf` is deleted from
`scripts/systemd/` along with the `_SUPERSET_OK` exemption that existed to let the live value exceed
its versioned baseline. Both halves of the SE1/SE2 pair are rebuilt for the file source as **SE6**:
what a root *declares* is read from the file, what is *in force* is read back from the running
daemon through `POST /api/overview`, because a file is re-read under an mtime cache and a project
whose config stopped parsing is quarantined rather than raised — so "in force" is only answerable by
the process. FE3, FE6 and FE12 are parametrised over both sources, and FE11's "lost" arm is now
`sources=()` rather than an empty env string, which is the experiment its own docstring conceded was
never available.
