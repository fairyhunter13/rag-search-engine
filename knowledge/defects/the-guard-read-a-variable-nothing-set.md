...
description: Two independent failures in one guard. The installer wrote a differently-named variable. Once that was fixed, it wrote a colon-joined value into a comma-split reader. The first is loud, the second is green.
...
it into three scopes under the engine's *previous* name. Nothing on the machine set the variable
the guard reads, so `terms()` failed correctly. That red is the whole point of an unset ban being
...
string, and semantically empty. That is the shape every mis-configured guard on this machine has
taken. `terms()` now refuses a token containing `:` or `;`. A company or device name does not
carry a separator, so a token that does is a list joined with the wrong one. Whitespace is deliberately
