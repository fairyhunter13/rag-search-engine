...
collapse to one watch descriptor. notify-rs reports events under whichever path was registered
**last**, measured both ways. Nothing in this code chooses the order. Unresolved, a member's
...
symlinked directory. Registering members under their resolved path is what avoids it. That
...
through inotify would test which of the two paths occurred to register last, which is the thing
nothing controls.

# Not the whole cause

The symptom came back — see
[a-delete-is-lost-while-a-project-is-still-settling](a-delete-is-lost-while-a-project-is-still-settling.md).
Both fixes here still hold and both were re-verified against real inotify. A delete through a
symlink reaches the queue under the member's resolved path, and the tick no longer re-arms an
unchanged set. What they do not cover is a delete issued while the project is still settling after
registration. That is the case the live test has, and the one that is still open.
