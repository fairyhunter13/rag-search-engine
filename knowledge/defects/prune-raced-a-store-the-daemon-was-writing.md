...
unlinked inode, and `commit()` returns clean. `registry.update` records chunk counts for a store
...
with no row that is not garbage. So a candidate written to in `CODERAG_PRUNE_MIN_IDLE_S` (60s)
...
The window was opened on the first `config.index_path` call. That call turned out to belong to
