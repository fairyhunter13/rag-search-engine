...
labelled with the project's own path. The walk lane refused the same file — the check has been
...
This is the same claim [the search unit is the caller's own workspace](../constraints/the-search-unit-is-the-callers-own-workspace.md)
makes about federation members, applied to file content: the indexer can reach the target, and that
is not a reason to index it. `read` refuses one too, so the refusal does not depend on which
