...
description: uvicorn waited on MCP's open streams, so systemd SIGKILLed at 90 s and fired OnFailure on every ordinary stop. The existing restart tests could not see it, because their stop helper kills after 30 s and reports success.
...
`_shutdown_exit` was dead code. That is the `os._exit(0)` that exists specifically so the CUDA
EP's static destructors cannot abort with 134. The process never chose how it died.
...
- **SIGKILL, not the controlled exit.** The one lifecycle fact the module docstring calls "bought
  with an outage" was not in force.
- **`OnFailure` fired on ordinary stops.** The alert unit waits 8 s and re-checks precisely so a
  clean restart does not page. A stop that always ends in `result 'timeout'` pages every time,
  which is how the alert that matters gets muted.
...
The new test drives the process directly rather than through that helper. It holds a live MCP
stream open while it signals. Without the stream there is nothing for uvicorn to wait on, and the
defect does not reproduce. The test asserts the exit code is 0, not merely that the process is
gone. Removing
...
default disposition, so the process dies by SIGTERM and the `os._exit(0)` below is dead code. The
module docstring calls the CUDA-134 protection a lifecycle fact "bought with an outage", and that
protection was not in force either. It occurred not to matter: a default-disposition SIGTERM does not run
...
`status=9/KILL` and `result 'timeout'` were a different thing fully, and this page's diagnosis of
*that* stands.
