...
description: Every plan in the chain verified with `coderag doctor reports no orphans`. It could not see 144 index directories, 436 MiB, whose rows were gone. A row-driven walk starts from a row, and these had none.
...
The guard test creates a store with no row *and* a disabled row with a store. It asserts `doctor`
...
claims leaves the registry **fully**, and nothing deletes its store. So every live run that
...
leaves the store. That is exactly how an unclaimed store is generated. The remaining half stands
