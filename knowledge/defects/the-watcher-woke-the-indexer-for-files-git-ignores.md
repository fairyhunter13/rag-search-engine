...
directory is in that repo's `.gitignore`, so `candidates()` never returns a file from it and every
...
`discover.git_ignored(project, rels)` — one `git check-ignore --stdin -z` per project per batch,
the same `--exclude-standard` semantics — and `_dispatch` subtracts it before submitting, only when
the project's config respects gitignore.
