...
The session fell back to `CPUExecutionProvider`. `verify_session` refused it. That is the only
...
This is the accurate class of failure the GPU-only invariant exists to catch, and it is the
argument for the fourth assertion specifically. The first three all passed. The EP ladder was non-empty, a
