...
description: The laptop's GPU runs at a sixth of rated clock. So every timing figure measured here measures the host rather than the engine. The cause is a power cap, not the heat the first reading blamed.
...
spend the rest. The chassis is saturated, and the CPU is at 96 °C competing for the same two fans.
...
* the **live suite's** `scoped_p50_ms` / `scoped_p95_ms`, recorded through `record_property` in
...
dissipate more heat, and it does not move 89 °C. The throttle costs roughly **6×**. That is an
...
between flushes until the card was under `CODERAG_INDEX_TEMP_C`. It was deleted on 2026-08-21 for
measuring at 0.04% of indexing self-time.

See [the thermal pause is not what indexing
costs](../decisions/the-thermal-pause-is-not-what-indexing-costs.md). Throttling is the driver's
now. The other half was never ours either — `nvidia-smi -pl` needs root, and 2026's power-capping
literature converges on ~70% TDP holding ~93% of throughput. A *deliberate* cap also beats this
involuntary one for measurement, because an irregular clock is what makes a number unrepeatable.
...
this shape does not read as a guess. It arrives with a unit and six important figures, and that is
...
The lever that exists is **`nvidia-powerd`**, the daemon that negotiates Dynamic Boost. Ubuntu
24.04's `nvidia-kernel-common-595` ships the binary at `/usr/bin/nvidia-powerd`. It installs
**neither** its systemd unit (doc-only, under `/usr/share/doc/`) **nor** any D-Bus policy. So the
service does not exist until it is assembled by hand.

Without the policy it starts and then fails to own `nvidia.powerd.server`. Both files placed, the
enforced limit went **80 W → 130 W** against a 175 W maximum. See [restoring Dynamic
Boost](../runbooks/restoring-dynamic-boost.md), which also names the `nvidia-smi` field that reads
`[N/A]` on a working system.

What this changes for measurement: **recall verdicts taken before the fix stay valid**. All arms
were capped equally and recall@k is not a timing metric, which is what the original entry got
right for the wrong reason. **Latency verdicts stay inadmissible**, and the reason changed under the fix.
...
runs into the thermal limit. The throttle is now the one the original entry named. At 87 °C,
against a 3090 MHz rating it still does not approach.

So the first amendment's correction was right about **why the card was slow in that window**. It
was wrong to imply heat was never a constraint here. The truthful statement is sequential: **power
was binding at 80 W. Heat is binding at 130 W**. A diagnosis that names one throttle reason is
only valid for the power limit it was taken at. Both readings above are correct at their own
limit.
