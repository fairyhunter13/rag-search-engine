...
description: "`last_error` is cleared by the hourly sweep, `last_error_at` is restamped by every failure and `error_total` never resets — so no single read of the registry separates a transient failure from a stuck project, and the checker holds the previous failing set instead."
...
`projects_failing > 0` would page for nearly every one of them, and
[the alert that cries wolf](../defects/a-failure-that-resolved-itself-left-no-trace.md) is how the
outage that mattered goes unread — the reason `alert_text` already sleeps eight seconds and
re-checks before paging for a restart.
...
- `error_total` is a lifetime count no success resets, by design. It says a project has failed, not
  that it is failing.

What is left is state the checker holds: a project pages only when it is in the failing set now and
was in it at the previous check. That is why `/healthz` returns `failing` — the identities — beside
...
claims a root under `tmp_path`, pytest deletes the directory, and the row survives with
`FileNotFoundError: ... is not a directory` — failing at every sweep, so it clears both samples.
That is the rule working, and it is also why `coderag doctor --prune` and an explicit release of the
dead rows belong at the end of a live run rather than whenever someone notices. Removal stays by
hand: `claim()` gets no path-shaped refusal, because `tmp_path` is where the registry's own tests
