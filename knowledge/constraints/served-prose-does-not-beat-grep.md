...
server configured. Asked a question whose answer was a literal string in this repo, it answered
with `grep`, correctly. It kept doing so through **three escalations**: two rewrites of the served
...
- in a repo the model can navigate, it guesses the filename from the layout and searches nothing
  at all.
...
directory. So that is a question the client's own tools cannot answer from the working tree. It is
also the capability this engine exists for, which makes it the fair contest rather than a rigged
one.
...
naming the thing grep cannot do: one working tree versus all of them. That name belongs in the
*tool description*, which is the text the model weighs at decision time. That is where the surviving sentence lives.
...
de-escalated on purpose. Reach for it when the question is about behavior rather than a literal
