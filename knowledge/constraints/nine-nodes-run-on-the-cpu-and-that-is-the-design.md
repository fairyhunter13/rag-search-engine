...
a property of one model, it is ORT declining to write a GPU kernel for an op whose inputs are shape
scalars. A kernel for those would cost two device copies to move an integer.
...
Together, a new op type trips the first and a big op hiding behind an old name trips the second. The
