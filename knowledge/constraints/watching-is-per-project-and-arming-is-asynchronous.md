...
description: The `index` reply's `watching` field used to be thread liveness. That is true the moment the watcher starts, and says nothing about whether this project's inotify watches exist. It is `watch.armed(project)` now, which is membership in the armed set. The rebuild lands up to WATCH_POLL_MS plus ~5 s after registration, and a write inside that window is gone for good.
...
wrote into exactly the window it was waiting out, and the live write test timed out at 300 s
again.
...
`index(...)["watching"]` first. It must not assert it synchronously right after `index` returns.
