...
- **Amendment**: `gpu.py`'s docstring said GPU-only is "asserted at four points". Three run in the daemon. `check_placement` has no production call site deliberately — reading it costs a profiled batch through both models, and what it guards changes on a model swap — so it is a gate held by `-m gpu`, which CI cannot run by decision. The docstring says that now, because the previous wording let the daemon's green read as covering it. `CLAUDE.md`'s 300-line ceiling likewise names `src/coderag/` and says four test files are over it, rather than being quoted as true in a fifth verification block.
