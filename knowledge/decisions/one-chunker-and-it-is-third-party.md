...
- **The one contradiction resolves.** [cAST's own paper](https://arxiv.org/html/2506.15655v1)
  reports +1.8 to +4.3 Recall@5 over line-based chunking. Its baseline is *naive* fixed-size line
  splitting with no boundary awareness at all; the 864-config study built a better window and the
  gap closed. cAST's delta is mostly the distance between naive and boundary-respecting, which is
  the distance a library with a boundary hierarchy already closes.
...
**non-whitespace characters** (the unit cAST defines and the 864-config study validates at 2,000);
...
Kept: the **scope header** — path, first import lines, nearest enclosing declaration — prepended to
the embedded and FTS text and never to the stored body. This is the mechanism the winning AST
chunker actually wins with (its own write-up credits the scope context, not the boundaries; cAST
has the same parser, lacks the header, and loses), obtained from a regex pass instead of a grammar
per language.
