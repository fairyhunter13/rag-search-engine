...
description: The bundle tests are a gate only if something runs them. So one arm reads the workflow. It fails when the step that invokes them is gone, excused, unpinned, or attached to a trigger that never fires on a change.
...
So one arm reads `.github/workflows/ci.yml` and requires five things. The step invoking these
tests still exists. The strict `-Werror` check runs beside it. The checker is installed at the
pinned version. No step is `continue-on-error`, because a swallowed failure reports the same green
as a pass. And a trigger fires on a change rather than a calendar.
...
Whether the workflow ran. That is a property of the forge, not of this checkout. Asking it here
