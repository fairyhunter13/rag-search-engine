...
Deleted: `CHUNK_HEADER_DERIVED`, its stamp field, and the `imports:` / `in: <decl>` /
heading-chain / key-path construction. Deleted with them: the five regexes behind those, and the
per-type dispatch that existed only to choose between them. `chunk.py` went 248 → 137 lines. `scope_header` takes one argument now.

# It was not redundancy, and that matters

The obvious explanation is that the header restates tokens already in the chunk body, so the
embedder sees nothing new. That explanation is wrong, and a census says so. Per chunk, the fraction of the derived line's tokens
...
The `imports:` line was **85% novel tokens** and bought nothing. So the embedder was handed
genuinely new text and did not use it. In the path-off cell the same text made retrieval *worse*
than emitting no header at all. That is dilution, not redundancy: the tokens compete with the chunk's own
...
structural metadata to a dense embedding, and it generalises. It also matches the one field
ablation in the literature: [arXiv:2601.11863](https://arxiv.org/abs/2601.11863) (ECIR 2026) finds
identity metadata carries the whole effect and *"removing section titles yields only a modest
drop"*. Path is identity, the derived line was section-title-class, on a different corpus with a
different embedder.

# The path arm, and the caveat that is not resolved

Kept, and the external corroboration is direct. Prepending the relative file path to each chunk
gave +17.0 pp Top-10 on SWE-bench-Lite, same mechanism and same direction
([arXiv:2605.17965](https://arxiv.org/abs/2605.17965), BLAgent). It reports 87.3% against 70.3%.

**But BLAgent's worked examples are query-document string identity, and it presents them
approvingly**. One case is a query naming `django.contrib.admin.utils` against a chunk headed
`django/contrib/admin/utils.py`. The other is a *stack trace* naming
...
source connects it to path-in-chunk, and neither ContextBench nor CORE-Bench ablates or masks
paths.

This repo already measured its own exposure and it is large: **48.6% of docs eval queries have
their heading echoed in the positive's filename** (`config.py`, `CHUNK_HEADER_PATH`). So part of
the docs path effect is a filename shortcut. What rescues the decision is that the *code* corpus
has no such echo. Its queries are docstring-derived, not filename-derived.

The path arm is worth −0.1233 recall@1 there anyway. The docs magnitude is soft. The code
magnitude stands.
...
The eval keeps a `no-header` arm. The path arm is now the only component, so an ablation that
cannot turn it off is not an ablation. This is the number the next person will want to re-check.
