"""Resumable driver: fully document all 24 federation members of redacted-name-3.
...
Usage:
  ...
  python scripts/document_federation.py --members acme-golibs acme-cart-be
...
async def enrich_root_until_complete() -> dict:
    """Enrich the redacted-name-3 root until all meaningful communities are done."""
...
async def run_status() -> None:
    ...
    print(f"\nredacted-name-3 federation: {len(members)} members\n")
    ...
    print("-" * 85)
    print(f"{'ROOT (redacted-name-3)':<60} {str(root_st['indexed']):>4} {root_st['communities']:>6} {root_st['enriched']:>7} {root_st['wiki_pages']:>5}")
