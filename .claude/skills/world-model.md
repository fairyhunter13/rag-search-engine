...
| P7 | Public-repo hygiene — absolute device paths never in wiki/OKF artifacts. |
...
| P12 | Doc-tooling (OKF) is LLM-native via claude -p; no tree-sitter on the doc-tooling path. Docgen's half was retired 2026-07-28 when docgen was deleted; OKF's leaves with the rest of tier 3. |
| P13 | OKF = manual-trigger only; never called from auto-sweep (_enrich_project) or MCP tools. (Docgen's half retired 2026-07-28 — docgen is deleted.) |
| P14 | LLM lanes: GPU=embed+rerank; DeepSeek=KB-enrichment; claude-haiku-4-5=chat via claude -p. The doc-tooling lane was deleted 2026-07-28 with docgen, so claude -p has exactly one caller (dashboard chat); the DeepSeek lane leaves with the rest of tier 3, after which this is two lanes. No cross-lane calls. |
| P15 | Kill-switch RSE_OKF=0 → no output; no deterministic skeleton fallback. (RSE_DOCGEN retired 2026-07-28 — no reader left.) |
...
| kb | `rag_search.kb` | enrich_communities_batch · build_wiki · reconstruct_processes · run_okf |
