...
| P2 | MCP query path (search/ask/graph/overview) runs NO generative LLM — only embed+rerank. The deepseek_* tokens stay in the check after tier 3's deletion: they now match nothing, which is the point — this is the reintroduction guard, not a description of a live call site. |
...
| P5 | Two-stage retrieval: hybrid recall — dense (sqlite-vec KNN) + lexical (FTS5 BM25 over chunks_fts), fused by RRF — then cross-encoder rerank (GPU). Results ordered by rerank_score, never by the RRF or vector score. |
