...
and require ~/git/github.com/fairyhunter13/redacted-name-3 to be present.
"""
# ruff: noqa: E402
...
# ---------------------------------------------------------------------------
# Large-project tests (requires OPENCODE_RUN_LARGE_TESTS=1 + redacted-name-3)
# ---------------------------------------------------------------------------

_ACME_PROJECT = os.path.expanduser(
    "~/git/github.com/fairyhunter13/redacted-name-3"
)
_RUN_LARGE = os.environ.get("OPENCODE_RUN_LARGE_TESTS", "").strip().lower() in {
    "1", "true", "yes",
}
_large = pytest.mark.skipif(
    not _RUN_LARGE or not os.path.isdir(_ACME_PROJECT),
    reason="set OPENCODE_RUN_LARGE_TESTS=1 and ensure redacted-name-3 is present",
)


def _acme_graph_has_data() -> bool:
    """Return True if the daemon has already built redacted-name-3's graph.db."""
    graph_db = Path(get_project_graph_db_path(_ACME_PROJECT))
...
@_large
@pytest.mark.large
async def test_e2e_acme_project_full_pipeline(tmp_path, monkeypatch):
    """
    Full pipeline on redacted-name-3 (meta-repo with 24 symlinked sub-repos).
    ...
    if not _acme_graph_has_data():
        ...
        result = await index_and_wait(_ACME_PROJECT)
...
@_large
@pytest.mark.large
async def test_e2e_acme_project_symlink_nodes_resolved(tmp_path, monkeypatch):
    ...
    try:
        ...
        symlinked_dir = os.path.join(_ACME_PROJECT, "repositories-ubuntu")
...
@_large
@pytest.mark.large
async def test_e2e_acme_project_cross_repo_edges(tmp_path, monkeypatch):
...
@_large
@pytest.mark.large
async def test_e2e_acme_project_global_search(tmp_path, monkeypatch):
    """global_search on redacted-name-3 returns results without errors."""
    from opencode_search.handlers._graph import handle_global_search
...
# ---------------------------------------------------------------------------
# Large test: discover federation from real redacted-name-3
# ---------------------------------------------------------------------------


@_large
@pytest.mark.large
async def test_e2e_acme_project_discover_federation(monkeypatch):
    """discover_federation finds all 24 symlinked sub-repos in redacted-name-3."""
    from opencode_search.handlers._federation import handle_discover_federation

    _use_real_registry(monkeypatch)

    result = await handle_discover_federation(project_path=_ACME_PROJECT)

    assert "error" not in result, f"discover_federation failed: {result}"
    # redacted-name-3 has 24 symlinks in repositories-ubuntu/
    ...
    # Verify the known acme-golibs path is discovered
    known = "/home/user/go/src/github.com/example-org/acme-golibs"
    assert known in result["discovered"], (
        f"acme-golibs not found in discovered: {result['discovered'][:5]}..."
    )
...
@_large
@pytest.mark.large
async def test_e2e_acme_project_pipeline_discover(monkeypatch):
    """pipeline() on redacted-name-3 discovers 24 federation members and registers them."""
    from opencode_search.handlers._pipeline import handle_pipeline
    ...
    acme_entry = registry.get(_ACME_PROJECT)
    assert acme_entry is not None
    assert len(acme_entry.federation) >= 20, (
        f"Federation should have 20+ members, got: {len(acme_entry.federation)}"
    )
