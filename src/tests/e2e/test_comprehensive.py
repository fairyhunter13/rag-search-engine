...
Tests all MCP tool functionality against the real redacted-name-3 index.
...
class TestT8StatusMetrics:
    ...
    def test_t8_1_project_status_indexed(self, monkeypatch):
        """P0: project_status shows redacted-name-3 as indexed with correct chunk count."""
        _use_real_registry(monkeypatch)
        from opencode_search.handlers import handle_project_status
        result = _run(handle_project_status(path=_ASTRO))
        assert result.get("indexed") is True, "redacted-name-3 not shown as indexed"
    ...
    def test_t8_2_list_indexed_projects_includes_astro(self, monkeypatch):
        """P0: list_indexed_projects includes redacted-name-3."""
        _use_real_registry(monkeypatch)
        ...
        assert _ASTRO in paths, f"redacted-name-3 not in indexed list: {paths[:5]}"
...
class TestT13Dashboard:
    ...
    def test_t13_api_communities_returns_list(self):
        """P0: GET /api/communities returns community list for redacted-name-3."""
...
class TestT23AutoPipeline:
    ...
    def test_t23_auto_pipeline_skips_already_enriched(self, monkeypatch):
        ...
        _use_real_registry(monkeypatch)
        # redacted-name-3 should already have enriched communities
        from opencode_search.handlers._autopipeline import _project_is_fresh
        # redacted-name-3 is known to be enriched — should NOT be fresh
...
'''
    ...
    @_LARGE
    def test_t24_acme_project_java_nodes_in_graph(self, monkeypatch):
        """P1: redacted-name-3 graph contains Java nodes from Spring Boot services."""
        _use_real_registry(monkeypatch)
        from opencode_search.config import get_project_graph_db_path
        from opencode_search.graph.storage import GraphStorage
        db_path = get_project_graph_db_path(_ASTRO)
        if not __import__("pathlib").Path(db_path).exists():
            pytest.skip("redacted-name-3 not indexed")
...
# ===========================================================================
# T26 — Full surface e2e against redacted-name-3 (all engine outputs validated)
# ===========================================================================
@_LARGE

class TestT26AstroProjectFullSurface:
    """P0: Comprehensive e2e validation of every engine output against redacted-name-3.

    Proves all MCP tools, dashboard routes, and auto-pipeline behaviour work
    correctly on the real indexed redacted-name-3 (20k files, 30+ federation members).
    """
    ...
    def test_t26_patterns_naming_and_frameworks(self, monkeypatch):
        """P0: Framework detection includes gRPC and Spring for redacted-name-3."""
        import urllib.parse
        _use_real_registry(monkeypatch)
        data = self._api(f"/api/patterns?project={urllib.parse.quote(_ASTRO)}")
        frameworks = [f.lower() for f in data.get("key_frameworks", [])]
    ...
    def test_t26_search_returns_go_code(self, monkeypatch):
        """P0: search finds Go handler code in redacted-name-3."""
        import urllib.parse
        _use_real_registry(monkeypatch)
    ...
    def test_t26_ask_architecture_returns_results(self, monkeypatch):
        """P0: ask(scope=architecture) returns community matches for redacted-name-3."""
        import urllib.parse
        _use_real_registry(monkeypatch)
    ...
    def test_t26_wiki_has_sufficient_pages(self, monkeypatch):
        """P0: Wiki has ≥100 pages for redacted-name-3."""
        import urllib.parse
        _use_real_registry(monkeypatch)
    ...
    def test_t26_graph_callers_resolves_for_known_symbol(self, monkeypatch):
        """P0: graph(relation=callers) works for a real Go function in redacted-name-3."""
        import urllib.parse
        _use_real_registry(monkeypatch)
        data = self._api(
    ...
    def test_t26_graph_export_has_nodes_and_edges(self, monkeypatch):
        """P0: graph_export returns nodes and edges for redacted-name-3."""
        import urllib.parse
        _use_real_registry(monkeypatch)
        data = self._api(
    ...
    def test_t26_java_nodes_in_graph_after_reindex(self, monkeypatch, tmp_path):
        ...
        member_path = None
        for candidate in [
            "/home/user/git/github.com/example-org/acme-api-customer-spring",
            "/home/user/git/github.com/example-org/acme-api-admin-spring",
        ]:
        ...
        assert member_path is not None, (
            ...
            "Expected acme-api-customer-spring or acme-api-admin-spring to exist."
        )
    ...
    def test_t26_dashboard_patterns_tab_has_go_primary(self, monkeypatch):
        """P0: Dashboard /api/patterns returns Go as primary language for redacted-name-3."""
        import urllib.parse
        _use_real_registry(monkeypatch)
        data = self._api(f"/api/patterns?project={urllib.parse.quote(_ASTRO)}")
        langs = data.get("languages", [])
...
# ===========================================================================
# T28 — Post-indexing KB completeness: proven against the real redacted-name-3.
#        Requires OPENCODE_RUN_LARGE_TESTS=1 and an already-indexed project.
# ===========================================================================

class TestT28PostIndexingKBProven:
    """P0/P1: Prove that after indexing the redacted-name-3, the full KB is built
    ...
    def test_t28_1_pipeline_events_recorded(self, monkeypatch):
        """P1: get_pipeline_events() has at least one event for the redacted-name-3."""
        _use_real_registry(monkeypatch)
    ...
    def test_t28_2_enrichment_communities_have_titles(self, monkeypatch):
        """P0: At least one community in the redacted-name-3 graph has an LLM-generated title.
    ...
    def test_t28_3_wiki_files_exist(self, monkeypatch):
        """P0: The wiki directory for the redacted-name-3 contains at least one .md file.
    ...
    def test_t28_4_patterns_cache_exists(self, monkeypatch):
        """P0: A patterns cache exists for the redacted-name-3.
    ...
    def test_t28_6_language_coverage_in_graph(self, monkeypatch):
        """P1: The redacted-name-3 has multi-language file stats from handle_detect_patterns.

        handle_detect_patterns returns a 'languages' list with {name, file_count}.
        The redacted-name-3 is a multi-language monorepo (Go, TypeScript, Python).
        """
        _use_real_registry(monkeypatch)
        from opencode_search.handlers import handle_detect_patterns
...
# ===========================================================================
# T30 — KB completeness vs redacted-name-3: all artifacts delivered and viewable.
#        Requires OPENCODE_RUN_LARGE_TESTS=1 and an indexed redacted-name-3.
# ===========================================================================

class TestT30KBCompletenessAstro:
    """P0/P1: Prove full KB delivery for the redacted-name-3:
