...
Requires: daemon at :8765, fully indexed redacted-name-3 federation (Workstream E done).
"""
...
def acme_root():
    p = next(
        (e.path for e in list_projects()
         if "redacted-name-3" in e.path and "promo" not in e.path and e.enabled),
        None,
    )
    assert p, "redacted-name-3 must be registered + enabled — run Workstream E first"
    return p


@pytest.fixture(scope="module")
def det_db(acme_root):
...
class TestDeterministicResolution:
    ...
    def test_process_db_created(self, acme_root):
        assert root_process_db(acme_root).exists(), "process_graph.db must be created by E"
    ...
    def test_deterministic_two_runs_same_count(self, acme_root):
