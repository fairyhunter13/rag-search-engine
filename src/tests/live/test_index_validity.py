...
class TestIndexValidity:
    ...
    def test_acme_root_has_process_graph(self, validate_reports: dict) -> None:
    ...
    def test_acme_process_edges_anchored(self, validate_reports: dict) -> None:
        pg = _chk(validate_reports, "astro").get("process_graph", {})
        assert pg.get("unanchored", 0) == 0, f"unanchored process edges: {pg}"

    def test_acme_process_edges_confidence_in_band(self, validate_reports: dict) -> None:
        pg = _chk(validate_reports, "astro").get("process_graph", {})
