...
def acme_root():
    p = next((e.path for e in list_projects() if "redacted-name-3" in e.path and e.enabled), None)
    assert p, "redacted-name-3 must be registered and enabled"
    return p
...
def acme_members(acme_root):
    ...
    assert len(members) >= 2, "redacted-name-3 must have ≥2 federation members"
    return members


@pytest.fixture(scope="module")
def process_db(acme_root):
    ...
    try:
        count = reconstruct_processes(acme_root)
    finally:
        if prev is None:
            os.environ.pop("OSE_WIKI_LLM", None)
        else:
            os.environ["OSE_WIKI_LLM"] = prev
    db = root_process_db(acme_root)
...
class TestLiveSurfaces:
    ...
    def test_H1_overview_returns_reconstructed(self, acme_root, process_db):
        import asyncio

        from opencode_search.server.mcp import overview as overview_tool
        loop = asyncio.new_event_loop()
        result = loop.run_until_complete(overview_tool(acme_root, what="process_flows"))
        loop.close()
    ...
    def test_H2_flows_have_mermaid(self, acme_root, process_db):
        import asyncio
    ...
    @pytest.mark.slow
    def test_H3_api_bpmn_endpoint(self, acme_root, process_db):
