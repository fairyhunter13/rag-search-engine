...
def astro(http):
    ...
    assert match.get("communities", 0) > 0, (
        "redacted-name-3 has no communities — run index(enabled=True) [daemon auto-builds KB] first"
    )
    return _ACME_PATH
...
def chat_cache(http):
    ...
    """
    from .test_acme_e2e import _chat  # lazy import avoids conftest<->test cycle
