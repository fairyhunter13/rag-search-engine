...
All tests are live — require the daemon at :8765 and redacted-name-3 indexed + enriched.
...
_PAYMENT_GW = "/home/user/go/src/github.com/example-org/redacted-name-0"
...
class TestCLIKbStatus:
    """opencode-search kb-status --json must report DONE for redacted-name-3."""
...
def _acme_members(http, astro) -> list[str]:
    """Return indexed federation member paths for redacted-name-3."""
    r = http.get("/api/federation", params={"project": astro, "action": "list"})
...
class TestAllFederationMembersConverge:
    """Every indexed redacted-name-3 federation member must verdict DONE.
    ...
    def test_all_members_done(self, http, astro):
        members = set(_acme_members(http, astro))
    ...
    def test_definitions_only_member_is_done(self, http, astro):
        ...
        assert zero_comm, (
            "No 0-community member found — expected ≥ 1 definitions-only repo (e.g. acme-proto)"
        )
...
class TestProjectKbIncompletePredicate:
    ...
    def test_payment_gateway_incomplete_when_zero_communities(self):
        """Gap A: redacted-name-0 with edges>0 but communities==0 → incomplete=True."""
        from pathlib import Path
        ...
        if not Path(db).exists():
            pytest.skip("redacted-name-0 not indexed")
        ...
        if edge_row is None:
            pytest.skip("redacted-name-0 has 0 edges — not Gap A scenario")
...
class TestVerdictWipedCommunityNotDone:
    ...
    def test_verdict_edges_but_zero_communities_not_done(self):
        ...
        if n_comms > 0:
            pytest.skip("redacted-name-0 already has communities — Gap A resolved; verdict may be DONE/PENDING depending on wiki")
