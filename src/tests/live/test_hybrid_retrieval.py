...
def test_lx1_backfill_never_runs_on_the_query_path(embedder, safe_tmp_path):
    ...
    Measured on the live fleet: opening `redacted-name-12` (99 k chunks) inside a query cost 11.31 s to then
...
def test_lx4_scope_filter_never_becomes_an_fts_rowid_constraint(embedder, safe_tmp_path):
    ...
    so the candidate set is nearly the whole corpus, and across redacted-name-10-project's 157 federation
