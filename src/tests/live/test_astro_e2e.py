"""Comprehensive end-to-end tests for redacted-name-3 via the live daemon.

Exercises every MCP tool and every major chat intent against the real redacted-name-3
knowledge base.  All tests require:
  - daemon running at :8765
  - redacted-name-3 indexed with communities > 0
...
redacted-name-3 (5896+ communities) will be selected over smaller projects.
"""
...
class TestAstroGraph:
    ...
    def test_graph_semantic_trace_returns_result(self, http, astro):
        """graph(semantic_trace) on redacted-name-3 must return a trace dict (not 5xx)."""
    ...
    def test_graph_path_returns_result(self, http, astro):
        """graph(path) on redacted-name-3 must return path or connected=false dict (not 5xx)."""
        symbol = self._find_a_symbol(http, astro)
        r = http.get("/api/graph", params={
...
class TestAstroOverview:
    ...
    def test_overview_projects_lists_astro(self, http, astro):
        ...
        assert any(_ASTRO in p for p in paths), (
            f"redacted-name-3 not listed in overview projects; found: {paths[:5]}"
        )
    ...
    def test_overview_process_flows(self, http, astro):
        """overview(what='process_flows') on redacted-name-3 returns valid dict."""
    ...
    def test_overview_business_rules(self, http, astro):
        """overview(what='business_rules') on redacted-name-3 returns valid dict."""
...
""",
    ...
    def test_symbol_intent_coverage_gte_90_pct(self, http, astro):
        ...
        assert jobs, "No enrich_symbols job found for redacted-name-3 — daemon should auto-submit on startup"
