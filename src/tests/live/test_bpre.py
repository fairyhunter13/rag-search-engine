...
def acme_campaign():
    p = next((p.path for p in list_projects() if "acme-campaign-be" in p.path and p.enabled), None)
    assert p, "acme-campaign-be must be registered and enabled"
    return p


@pytest.fixture(scope="module")
def acme_promo():
    p = next((p.path for p in list_projects() if "acme-promo-be" in p.path and p.enabled), None)
    assert p, "acme-promo-be must be registered and enabled"
    return p
...
class TestPhase0VocabularyAndBackfill:
    ...
    @pytest.mark.slow
    def test_semantic_type_coverage_campaign(self, acme_campaign):
    ...
    @pytest.mark.slow
    def test_semantic_type_coverage_promo(self, acme_promo):
    ...
    @pytest.mark.slow
    def test_business_process_communities_exist(self, acme_campaign):
    ...
    @pytest.mark.slow
    def test_business_rule_communities_exist(self, acme_promo):
    ...
    @pytest.mark.slow
    def test_overview_business_rules_returns_content(self, acme_promo):
    ...
    @pytest.mark.slow
    def test_overview_process_flows_returns_content(self, acme_campaign):
    ...
    def test_overview_business_rules_matches_db(self, acme_promo):
    ...
    @pytest.mark.slow
    def test_ask_scope_business_returns_business_communities(self, acme_campaign):
        """P1.4: ask(scope='business') must return context with business community content."""
        ctx = asyncio.run(ask_tool("what are the main business processes?", acme_campaign, "business"))
...
class TestNegativeCases:
    ...
    @pytest.mark.slow
    def test_process_flows_summaries_not_empty(self, acme_campaign):
...
class TestPipelineIdempotency:
    ...
    @pytest.mark.slow
    def test_backfill_preserves_existing_summaries(self, acme_promo):
    ...
    @pytest.mark.slow
    def test_backfill_idempotent_twice(self, acme_campaign):
    ...
    @pytest.mark.slow
    def test_ask_all_scope_still_works_after_vocab_fix(self, acme_campaign):
        """I3: ask(scope='all') must not regress after vocabulary fix deployment."""
        ctx = asyncio.run(ask_tool("how does indexing work?", acme_campaign, "all"))
...
class TestLLMOutputStability:
    ...
    @pytest.mark.slow
    def test_semantic_types_stable_between_reads(self, acme_promo):
    ...
    @pytest.mark.slow
    def test_overview_business_rules_count_stable(self, acme_promo):
...
class TestRegressionGuard:
    ...
    def test_feature_map_still_works(self, acme_promo):
        """R1: overview(what='feature_map') must still work after vocab fix."""
        result = asyncio.run(overview_tool(acme_promo, "feature_map"))
        data = json.loads(result)
        assert "features" in data, f"feature_map broken after vocab fix: {data}"

    def test_communities_still_work(self, acme_campaign):
        """R2: overview(what='communities') must return >=10 communities."""
        result = asyncio.run(overview_tool(acme_campaign, "communities"))
        data = json.loads(result)
    ...
    @pytest.mark.slow
    def test_business_rules_now_non_empty(self, acme_promo):
...
class TestClassificationStability:
    ...
    @pytest.mark.slow
    def test_daemon_classification_is_stable(self, acme_campaign):
        ...
        """
        gs0 = GraphStore(project_graph_db(acme_campaign))
        try:
