...
def _find_import_cycles(conn) -> list[list[str]]:  # type: ignore[no-untyped-def]
    ...
    (the §8 measurement: 0% of mcsv-kpi's import pairs were induced by calls) and a call can
...
def handle_overview(project_path: str, what: str, query: str = "") -> str:
    ...
    if project_path:
        ...
        def _each_store():  # type: ignore[no-untyped-def]
            ...
            WAL connection is three descriptors (db + -wal + -shm), so on inosoft-project's 157
