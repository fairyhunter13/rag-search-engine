...
# the same commit, and the bump is what invalidates every stored graph in the fleet. A dependency
# version counts: `e9` moved for a grammar-pack upgrade with no code change here.
#
# What each rev did, and what it measured: `docs/decisions/2026-08-14-the-extractor-rev-log.md`.
EXTRACTOR_REV = "e14"
...
class Symbol:
    ...
    end_line: int
    language: str
    # Every field here must reach a column: `upsert_symbol` takes it and the store keeps it. A
    # field written and never read is a claim the schema does not honour, and SC6 fails on it.
...
# Call extraction walks `_iter_script_blocks` and deliberately not `_embedded_blocks`: widening it
# to rung-1 injections adds 197 call names fleet-wide and every one is CSS (`var` ×158, `rgba` ×24),
# because html statically injects css and `var(--x)` is a call node there. Measured 2026-07-30 —
# docs/decisions/2026-07-31-call-extraction-skips-injections-deliberately.md.
