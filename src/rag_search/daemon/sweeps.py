...
def _code_scan(path: str) -> tuple[str, float]:
    ...
    # was 9.1 hours older than its newest file, inosoft's 1.5 hours — and since the baseline
...
def _index_set_drift(path: str) -> tuple[list, list[str]]:
    ...
    inosoft's 5,242 indexed paths had no hash row at all, written by an index generation that
