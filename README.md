...
**`index(root="", enabled=True)`** — registers the root, finds the repos it symlinks, registers
...
string. `mode="semantic"` is for a question in English. The default fuses both with RRF and a
cross-encoder.

An unknown `mode` or `lang` is an error naming the valid set, never a widened corpus. `root` names
one root. The federation expansion is the engine's job.
...
through the existing alert unit. `health` exits non-zero only for a project that was failing at
the previous check as well. `last_error` clears on the hourly sweep, so a checker keyed on one
sample pages for every transient failure.

`doctor --prune` is the only destructive subcommand. It deletes store directories no registry row
claims, and holds the registry lock so nothing can claim one while it looks. It keeps any store
written to in `CODERAG_PRUNE_MIN_IDLE_S` (60s), because that is a row-less job still finishing.
...
No mocks: a test either calls the real model on the real GPU or touches no model at all. The
public hygiene guard needs `CODERAG_NAME_BAN` set. `=none` for a clean clone, because a guard that
